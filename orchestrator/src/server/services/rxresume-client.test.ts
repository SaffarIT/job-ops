  ...
  describe("verifyCredentials (static)", () => {
    ...
    it("returns ok: false with statusMessage from response", async () => {
      const mockFetch = vi.fn().mockResolvedValue({
        ...
        status: 403,
        text: async () =>
          JSON.stringify({ statusMessage: "Account suspended" }),
      });
      vi.stubGlobal("fetch", mockFetch);
  ...
  describe("instance methods", () => {
    ...
    describe("login", () => {
        ...
        const mockFetch = vi.fn().mockResolvedValue({
          ...
          headers: {
            ...
            getSetCookie: vi
              .fn()
              .mockReturnValue([
                "Authentication=cookie-token; Path=/; HttpOnly",
              ]),
          },
          json: async () => ({}),
        });
      ...
      it("throws error on login failure", async () => {
        ...
        await expect(
          client.login("wrong@example.com", "badpass"),
      ...
      it("throws error when token is not found in response", async () => {
        ...
        ).rejects.toThrow("could not locate access token");
      });
    });
    ...
    describe("delete", () => {
      it("completes successfully on 200 response", async () => {
        ...
        await expect(
          client.delete("resume-123", "mock-token"),
